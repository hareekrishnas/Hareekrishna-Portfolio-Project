...
For the component representation, I choose to model my component using a map
data structure from the OSU API. I need a map data structure to make it easier to
do my operations like adding, removing, and even looking at my spells in the spellbook
fast. For convention, the map cannot be null and the power levels cannot be
negatives. For correspondence, the size of the map equals the amount of spells
in the SepplBook and if a name is not present in the Spell names, then it
is not in the book.
